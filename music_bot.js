...
class Music {
    ...
    playMusic(msg, guildID, musicInfo) {
        ...
        // 提示播放音樂
        if(this.yt_play_mode != "loop_one_mode"){
        ...
        this.dispatcher[guildID].on('finish', () => {
            ...
            //移除 queue 中目前播放的歌曲
            msg.channel.send(`AAAAA: ${this.queue[guildID][0]}`);
            switch(this.yt_play_mode){
                case "normal_mode":
                    this.queue[guildID].shift();
                    msg.channel.send(this.yt_play_mode);
                    break;
                case "loop_one_mode":

                    break;
                case "loop_queue_mode":
                    msg.channel.send("queue looping");
                    this.queue[guildID].push(this.queue[guildID].shift());
                    break;    
    ...
    loop(msg){
        if (this.dispatcher[msg.guild.id]) {
            msg.channel.send('call loop');

            // 變更成loop_one模式
            if(this.yt_play_mode == "loop_one_mode"){
                this.yt_play_mode = "normal_mode";
                msg.channel.send(`call loop : ${this.yt_play_mode}`);
            }else{
                this.yt_play_mode = "loop_one_mode";
                msg.channel.send(`call loop : ${this.yt_play_mode}`);
            }
    ...
    loop_queue(msg){
        if (this.dispatcher[msg.guild.id]) {
            msg.channel.send('call loop_queue');

            // 恢復播放
            if(this.yt_play_mode == "loop_queue_mode"){
                this.yt_play_mode = "normal_mode";
                msg.channel.send(`call loop queue : ${this.yt_play_mode}`);
            }else{
                this.yt_play_mode = "loop_queue_mode";
                msg.channel.send(`call  queue loop : ${this.yt_play_mode}`);
            }
    ...
    rm(msg) {
        ...
        if (this.dispatcher[msg.guild.id]) {
            msg.channel.send('摧毀單位!!!');
            const rm_id = msg.content.replace(`${prefix}rm`, '').trim();
            msg.channel.send(`id = ${rm_id}`);
            // 恢復播放
            this.queue[msg.guild.id].splice(rm_id-1,1);
        }
    ...
    nowQueue(msg) {
        ...
        if (this.queue[msg.guild.id] && this.queue[msg.guild.id].length > 0) {
            // 字串處理，將 Object 組成字串
            const queueString = this.queue[msg.guild.id].map((item, index) => `\n[${index+1}] ${item.name}`).join();
...
client.on('message', async (msg) => {
    ...
    // !!qloop
    if (msg.content === `${prefix}qloop`) {

        // 重複list
        music.loop_queue(msg);
    }
    // !!rm
    if (msg.content.indexOf(`${prefix}rm`) > -1) {

        // rm
        msg.channel.send('check rm');
        music.rm(msg);
    }
